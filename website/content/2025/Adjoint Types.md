...
One possibility is we don't notice this mistake until we run our code, and then we get a runtime error telling us we did something wrong. This is what would happen in a language such as python with no static type checker. Static type systems allow this error to be caught before runtime. We can specify rules that all programs must follow and statically (without running the code) determine whether the code follows them. For example, we could define a rule for functions that says that if we know the function should take in type A and output type B, then we can only apply it to something of type A (and we will then be guaranteed that, if the function terminates, the output will have type B). 
...
We define modes via the keyword *mode* followed by the name we want to give it, a keyword that states what structural properties the mode has, and any relationship we want this mode to have with other declared modes. All our examples will consider the following two modes:
...
This states that the mode \\(L\\) is linear (admits neither weakening nor contraction), and the mode \\(U\\) is unrestricted (admits both weakening and contraction). The \\(U :> L\\) establishes the relationship that data at mode \\(U\\) cannot rely on data at mode \\(L\\), but data at mode \\(L\\) can rely on data at mode \\(U\\) and that \\(U\\) must be at least as permissive as \\(L\\). It is also possible to define modes that admit weakening but not contraction using the affine keyword and modes that admit contraction but not weakening using the strict keyword, but here we stick with just these two for simplicity.


## Defining Types
We define types via the keyword *type* followed by the name of the type and a mode annotation.

To start with, we define unary natural numbers. Note that, in this language, we require all recursive types to be guarded. In the examples here, we will see them guarded by a down shift from one mode to itself. The computational interpretation of a down shift is a pointer.

```
type nat[m] = +{'zero : 1, 'succ : <nat[m]>}
```

The annotation \\([m]\\) says that the type ``nat`` has the mode \\(m\\), the + syntax denotes a sum type. That is, nat can be either zero, which has the unit type, or the successor of another natural number (wrapped in a pointer).
...
From the side of optimizations, it turns out that, if ``l1`` is linear, we can immediately reuse the memory that it was stored in for the first part of the combined list, tagging on ``l2`` as the tail once we reach the end of ``l1``. 
If ``l1`` is unrestricted however, this optimization is not valid, as ``l1`` may be used again somewhere else, and needs to exist in its original state. Note however that while the memory representation may change, the actual value of the output produced by the append definition will not change regardless of the mode of the inputs.
